...
class AMBF_PT_main_panel(Panel):
    ...
    def draw(self, context):
        ...
        for index, collection in enumerate(scene.ambf_collection_properties):
            ...
            col.prop(collection, 'meshes_save_type')
            row = sbox.row()
            row.prop(collection, 'save_high_res')
            row.prop(collection, 'save_low_res')
            row.prop(collection, 'save_textures')
            col = sbox.column()
            col.prop(collection, 'save_selection_only')
            ...
            col = sbox.column()
            col.prop(collection, "model_override_gravity")
            col.prop(collection, "model_gravity")
            col.prop(collection, "ignore_inter_collision")
            col.prop(collection, "precision")
            col.prop(collection, 'namespace', text='Global NS')
            col.prop(collection, 'adf_path', text='Save As')
            col.operator("ambf.add_generate_ambf_file")

        # # Dropdown Menu for Collections
        # row = box.row()
        # row.label(text="Select Collection to Operate:")

        # row = box.row()
        # row.prop(context.scene, "selected_collections", text="Collection")

        # collection = bpy.data.collections.get(context.scene.selected_collections)
...
def register():
    ...
    bpy.utils.register_class(AMBFCollectionProperties)
    bpy.types.Scene.ambf_collection_properties = CollectionProperty(type=AMBFCollectionProperties)
    bpy.types.Scene.collection_enum = bpy.props.EnumProperty(items=update_collection_enum_items, name="Collections")
...
def unregister():
    ...
    bpy.utils.unregister_class(AMBFCollectionProperties)
